...
#[dl_scripts/detection/yolo2coco.py](https://github.com/Weifeng-Chen/dl_scripts/blob/main/detection/yolo2coco.py)
...
def train_test_val_split_random(img_paths,ratio_train=0.8,ratio_test=0.1,ratio_val=0.1):
    ...
    print(math.isclose(ratio_train+ratio_test+ratio_val,1.0))
    assert math.isclose(ratio_train+ratio_test+ratio_val,1.0)
...
def yolo2coco(arg):
    ...
    if arg.random_split or arg.split_by_file:
        ...
        if arg.random_split:
            ...
            ratios=[ float(i) for i in arg.split_ratio.split(':')]
            if len(ratios)!=3:
                raise ValueError("wrong split_ratio input")
            print("have assingn ratio {}:{}:{}".format(ratios[0],ratios[1],ratios[2]))

            train_img, val_img, test_img = train_test_val_split_random(indexes,ratios[0],ratios[1],ratios[2])
        ...
        with open(os.path.join(originLabelsDir, txtFile), 'r') as fr:
            ...
            for label in labelList:
                ...
                dataset['annotations'].append({
                    ...
                    # 'segmentation': [[x1, y1, x2, y1, x2, y2, x1, y2]] #buggy
                    'segmentation':[],
                })
                ann_id_cnt += 1
